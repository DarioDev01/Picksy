...
// ─── CONFIGURA AQUÍ TU TAG DE AMAZON ASSOCIATES ──────────────────────────────
const AFFILIATE_TAG = "your-tag-20"; // ← Reemplaza con tu tag (ej: "mitienda-20")
// ──────────────────────────────────────────────────────────────────────────────
...
export default function App() {
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "¡Hola! Soy tu asesor de compras con IA. Dime qué necesitas y te encontraré las mejores opciones en Amazon. 🛒",
      products: []
    }
  ]);
  ...
  async function sendMessage() {
    ...
    try {
      ...
      // Obtenemos el modelo del .env o usamos uno por defecto
      const selectedModel = import.meta.env.VITE_CLAUDE_MODEL || "claude-3-5-sonnet-20241022";

      // Llamada a /api/chat — en dev hace proxy a Anthropic, en producción usa Vercel Function
      ...
      const raw = data.content?.[0]?.text || '{"text":"Lo siento, algo salió mal.","products":[]}';
      ...
      setMessages(prev => prev.map(m =>
        m.id === loadingId
          ? { role: "assistant", text: `Error: ${err.message || "No se pudo conectar con la IA. Por favor intenta de nuevo."}`, products: [] }
          : m
      ));
    } finally {
      setLoading(false);
  ...
  const suggestions = ["Auriculares gaming", "Silla ergonómica de oficina", "Tableta gráfica", "Cafetera eléctrica"];
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)", margin: 0 }}>
            Recomendaciones personalizadas · Links de Amazon con afiliado
          </p>
        </div>
        ...
        <p style={{
          ...
          Como Asociado de Amazon gano por compras calificadas · Links incluyen tu tag de afiliado
        </p>
      </div>
    </div>
  );
